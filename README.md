...
Linux 배포 환경의 OP.GG·tactics.tools 크롤러에는 Chrome/Chromium과 호환되는 ChromeDriver가
필요합니다. 두 실행 파일을 PATH에 두거나 `CHROME_BIN`, `CHROMEDRIVER_PATH`로
경로를 지정하세요. 크롤러는 `--headless=new`와 독립 프로필을 사용하며,
