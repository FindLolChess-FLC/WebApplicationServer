...
# 수집 댓글의 출처를 기록할 테이블을 한 번 생성합니다.
python manage.py migrate Crawling

# 롤체지지의 일반 덱 설명을 덱당 한 개의 슈퍼관리자 댓글로 저장합니다.
# 이전 버전의 출처 표시·문단별 댓글은 한 개의 설명으로 정리합니다.
...
출처별 슈퍼관리자 댓글을 덱당 한 개씩 저장하고 재실행 시 갱신합니다. 일반 사용자 댓글은 건드리지
않습니다. OP.GG는 현재 덱별 설명이 없어 저장을 건너뛰고, tactics.tools의
짧은 전략 태그는 제목과 챔피언 구성이 일치하는 기존 메타에만 연결합니다.
이 명령은 이미지 업로드나 로컬 파일 저장을 하지 않습니다.
새 댓글 내용에는 출처 표시가 들어가지 않고 문단 줄바꿈은 유지됩니다. 별도
`Crawling` 테이블이 수집 댓글을 기록하므로 중복 없이 재수집할 수 있습니다.
웹 화면에서도 줄바꿈을 보이려면 댓글 본문에 `white-space: pre-line`을 적용하세요.
